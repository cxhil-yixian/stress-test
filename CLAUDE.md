...
8. **報告是單一檔案，五項固定 CPU→RAM→DISK→SWAP→NTP。**
   `LOG` 由主流程設定一次，`t_*` 不再各自開檔。章節編號寫死 `n/5` 的正式順序，跟實際跑了哪幾項無關。

   摘要用 `SUM_CPU` / `SUM_RAM` / `SUM_DISK` / `SUM_SWAP` / `SUM_NTP` 五個全域，預設值是「未執行」，每個 `t_*` 跑完自己填。**不要把沒跑到的項目從摘要裡拿掉** ——「沒測」和「沒問題」看起來一樣就糟了，這是這份報告的重點之一。

   要撈某一段的監看數據，用 `m=$(mark)` 記下行號再 `since "$m" | ...`。報告是單一檔案，直接 grep 整個 `$LOG` 會把前面項目的數據也算進來。

9. **新增測試項目時的模式：**
   `sec <n/5> <名稱>` → `need <工具...> || { SUM_XXX="跳過 (缺工具)"; return 1; }` → `log` 說明參數 → `m=$(mark)` → `mon_start _mon_xxx` → 主壓測（`2>&1 | tee -a "$LOG"`）→ `mon_stop` → 從 `since "$m"` 撈數字填 `SUM_XXX`。注意 `sec` 要在 `need` 之前，缺工具的訊息才會落在正確的章節底下。同時更新 `usage()` 裡的 here-doc 與 `case` 分支。

...
- README 重排：「測試指令（複製即用）」移到最前面，本機與網路測試的完整 `bash <(curl ...)` 一鍵指令集中在開頭，複製整段即可執行；說明性內容（為什麼用 `bash <(...)`、分支名、存成檔案、輸出位置、環境變數、判讀）一律往下移。指令用第一行 `S=<raw URL>` 存網址、後續共用 `$S`，避免長網址重複洗版

### 修正（網路測試）

- **curl 下載量在下載逾時（大檔）時被歸零。** `_dl_worker` 用 `curl ... || got=0`，但大檔在 `--max-time 30` 內下不完時 curl 以 exit 28 結束，`|| got=0` 就把那 30 秒實際下載的幾十 MB 全丟掉。Cloud180 實測：網卡收了約 55MB，摘要卻回報 `curl 實際下載 21KB`。逾時本來就是「持續灌流量」的正常狀態，`--write-out` 的 `size_download` 照樣會印，改成不看 exit code、只驗數字。（先前的本地測試用秒下完的小檔，走的是 exit 0 路徑，剛好遮住這個 bug）
- README 補充：下載速率受「到來源的鏈路」限制，來源太遠會量到跨洲上限而非單機接收能力；接收速率又低又平、CPU 又閒時多半是來源太遠
